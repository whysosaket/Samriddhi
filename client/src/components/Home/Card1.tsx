const Card1 = () => {
  return (
    <div
      className="relative m-auto select-none z-50 h-48 w-80 rounded-xl opacity-60 bg-gradient-to-r from-orange-500/50 to-red-500/0 metallic-text shadow-2xl transition-transform sm:h-56 sm:w-96 sm:hover:scale-110"
    >
      <div className="absolute top-4 w-full px-8 sm:top-8">
        <div className="flex justify-between">
          <div className="">
            <p className="font-light bg-gradient-to-r from-white to-gray bg-clip-text text-transparent">
              Name
            </p>
            <p className="font-medium tracking-widest bg-gradient-to-r from-white to-gray bg-clip-text text-transparent">
              Carter Mullen P
            </p>
          </div>
          <img
            className="h-14 w-14 object-contain"
            src="data:image/png;base64,"
          />
        </div>
        <div className="pt-1">
          <p className="tracking-more-wider font-medium bg-gradient-to-r from-white to-gray bg-clip-text text-transparent">
            0006 2345 7453 2345
          </p>
        </div>
        <div className="pt-4 pr-6 sm:pt-6">
          <div className="flex justify-between">
            <div className="">
              <p className="text-xs font-light bg-gradient-to-tr from-white to-gray bg-clip-text text-transparent">
                Valid From
              </p>
              <p className="text-base font-medium tracking-widest bg-gradient-to-tr from-white to-gray bg-clip-text text-transparent">
                11/15
              </p>
            </div>
            <div className="">
              <p className="text-xs font-light bg-gradient-to-tr from-white to-gray bg-clip-text text-transparent">
                Expiry
              </p>
              <p className="text-base font-medium tracking-widest bg-gradient-to-tr from-white to-gray bg-clip-text text-transparent">
                03/25
              </p>
            </div>
            <div className="">
              <p className="text-xs font-light bg-gradient-to-tr from-white to-gray bg-clip-text text-transparent">
                CVV
              </p>
              <p className="tracking-more-wider text-sm font-bold bg-gradient-to-tr from-white to-gray bg-clip-text text-transparent">
                521
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Card1;
